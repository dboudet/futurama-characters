export default function Results({char}) {
    ...
    return(
        <div className="results">
            ...
            {char.name && <h2>{name.first} {name.middle} {name.last}</h2>}
            ...
            {char.sayings?.length > 0 && <p><strong>Sayings:</strong><br/>{sayings.slice(0,5).map( (saying) => <span className="saying">{saying}<br/></span>)}</p>}
        </div>
        )
}